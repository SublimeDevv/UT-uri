...
export default function Msubcategoria() {
  ...
  const subir = async () => {
    ...
    if (body.nombre && body.info) {
      ...
      try {
        const respuesta = await axios.post("http://localhost:8081/api/subcategorias/CrearSubcategoria", { Nombre, Descripcion });
        ...
        if (respuesta.data.Estatus === "EXITOSO") {
          ...
          Swal.fire("Etiqueta creada con éxito", "success");
        }
      } catch (error) {
        console.error(error);
      }
